...
<script type="text/javascript">
	...
	//改变交易阶段stage：需要改变的阶段，i需要改变的阶段对应的下标
	...
	function changeIcon(stage,index1)
	{
		...
		alert("当前阶段的下标"+index);
		alert("分界点下标"+point);*/
		//如果当前阶段的可能性为0，前7个一定是黑圈，后两个，一黑叉一红叉
		if(currentPossibility=="0")
		{
			for (var i=0;i<point;i++)//遍历前面7个
			{//黒圈
				//移除原来的样式
				$("#"+i).removeClass();
				//添加新样式
			...
			//遍历后面两个
			for (var i=poitn;i<<%=dvList.size()%>;i++)
			{
				if (i==idnex)//如果是当前阶段
				{//红叉
					//移除原来的样式
		...
		}else {//如果当前阶段可能性不为0,前面7个可能是绿圈，绿色标记，黑圈，后两个一定是黒叉
			//遍历前面7个
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//判断当前的可能性
			//如果当前的可能性为0，前面7个一定是黑圈，后两个一个是红叉，一个黒叉
			if ("0".equals(currentPossibility))
			{
				for (int i=0;i<dvList.size();i++)
				{
					...
					//如果遍历出来的可能性值为0,说明是后两个，一个是红叉，一个黒叉
		...
		%>
						<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
							  class="glyphicon glyhpicon-remove mystage"
		...
		<%
				...
				//如果当前可能性不为0，前面7个有可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
			}else {
				//准备当前阶段的下标
				int index=0;
				for (int i=0;i< dvList.size();i++)
		...
		%>
					...
					-----------
		<%
						//如果遍历出来的可能性值不为0，说明是前7个阶段，黑圈，绿圈，绿色标记..
					}else {
						//如果是当前阶段
						if (i==index){
							//绿色标记----------
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 40px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>${t.activityId}</b></div><%--这里的activityId表示市场活动名--%>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 50px;">
			<div style="width: 300px; color: gray;">联系人名称</div>
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${t.contactsId}</b></div><%--这里的contactsId表示联系人名--%>
