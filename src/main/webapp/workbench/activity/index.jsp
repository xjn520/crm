...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#addBtn").click(function (){
			...
			$.ajax({
				...
				success:function (data){
					...
					//将当前登录系统用户，作为下拉列表的默认选项
					var id="${sessionScope.user.id}"
					$("#create-marketActivityOwner").val(id);
					//第二部，弹出模态窗口
					$("#createActivityModal").modal("show");  //show表示打开，hide表示关闭
				}
			})
		})
		//为保存按钮添加操作
		...
		pageList(1,2);
		//为查询按钮绑定时间，出发pageList方法
		$("#searchBtn").click(function (){
			//点击查询按钮的时候，应该将搜索框中的数据保存到隐藏域当中
		...
		$("#deleteBtn").click(function (){
			//找到复选框所有选√的
			var $xz=$("input[name=xz]:checked"); //以$开头的变量是jQuery对象
			if ($xz.length==0)
			{
				alert("未选中任何数据");
			}else {
				...
				if (confirm("确定删除吗？")){
					...
					{
						param+="id="+$($xz[i]).val()+"&";//取出来的dom对象又包装成jQuery对象
					}
					param=param.substring(0,param.length-1);//去掉字符串中最后的一个&符号
					$.ajax({
		...
		$("#editBtn").click(function (){
			...
			}else if ($xz.length>1)
			{
				...
				var id=$xz.val();
				$.ajax({
					url:"workbench/activity/getUserListandActivity.dao",
					data:{
						"id":id
					},
					type:"get",
	...
	* */
	function pageList(pageNo,pageSize)
	{
		...
		$.ajax({
			...
			//	data:[{市场活动1},{市场活动2}]
				var html="";
				//a每一个element都是一个json对象（市场活动对象）
				$.each(data.dataList,function (index,element){
					html+='<tr class="active">';
					html+='<td><input type="checkbox" name="xz" value="'+element.id+'"/></td>';
					html+='<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'workbench/activity/detail.dao?id='+element.id+'\';">'+element.name+'</a></td>';
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
				<div class="btn-group" style="position: relative; top: 18%;">
					<%--
					data-toggle="modal" 表示出发该按钮，会弹出一个模态的窗口
